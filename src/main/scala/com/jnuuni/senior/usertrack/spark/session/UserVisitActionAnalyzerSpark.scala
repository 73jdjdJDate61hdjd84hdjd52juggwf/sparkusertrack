package com.jnuuni.senior.usertrack.spark.session

import java.lang
import java.sql.Connection

import com.alibaba.fastjson.{JSONArray, JSONObject}
import com.jnuuni.senior.usertrack.conf.ConfigurationManager
import com.jnuuni.senior.usertrack.constant.Constants
import com.jnuuni.senior.usertrack.dao.factory.DAOFactory
import com.jnuuni.senior.usertrack.jdbc.JDBCHelper
import com.jnuuni.senior.usertrack.mock.MockDataUtils
import com.jnuuni.senior.usertrack.spark.util.{JSONUtil, SQLContextUtil, SparkConfUtil, SparkContextUtil}
import com.jnuuni.senior.usertrack.util.DateUtils.DateTypeEnum
import com.jnuuni.senior.usertrack.util.{DateUtils, ParamUtils, StringUtils}
...
object UserVisitActionAnalyzerSpark {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //1.2根据taskid从数据库中获取任务对象
    /**
      * 方式：
      * 1. 基于原始的JDBC/ODBC的代码实现数据的获取
      * 2. 集成第三方的框架, eg: mybatis....
      * 3. 使用SparkSQL来读取JDBC的数据(底层也是第一种方式)
    ...
    // 2.2 SparkConf的创建
    val conf = SparkConfUtil.generateSparkConf(appName, isLocal, setupSpecialConfig = that => {
      //这里可以单独的指定当前spark应用的相关参数
      //nothings
    })
    // 2.3 SparkContext对象的创建
    val sc: SparkContext = SparkContextUtil.getSparkContext(conf)
    // 2.4 SQLContext对象的创建
    /**
      * 实际运行环境中，数据一定是在Hive表中，所以一定要构建HiveContext对象；但是在开发测试过程中，由于连接
      * hive一般麻烦，而且执行速度比较慢；所以经常在开发过程中，使用模拟数据进行测试
      */
    ...
    // 计算得到：每个会话的sessionId和这个sessionId的会话时长的二元组
    ...
    //计算会话不同级别(A/B/C/D/E/F/G)和对应的个数的二元组
    val preSessionLengthLevelSessionCount: Array[(String, Int)] = sessionID2LengthRDD
      .map {
        case (_, length) => {
          // 根据会话长度得到当前会话的级别
          val sessionLevel = {
    ...
    val dayAndHour2SessionLengthRDD: RDD[((String, Int), Long)] = sessionID2RecordsRDD.flatMap {
      case (sessionID, records) => {
        ...
        //val kdfj: Map[(String, Int, String), Iterable[((String, Int, String), Long)]] = dayAndHourAndSessionID2TimestampIter.groupBy(_._1)
        // 2. 计算各个时间段各个会话中的会话长度
        val dayAndHour2LengthIter: Map[(String, Int), Long] = dayAndHourAndSessionID2TimestampIter
          .groupBy(_._1)
          .map {
            case ((day, hour, sessionID), iter) => {
              val times: Iterable[Long] = iter.map(_._2)
              // a. 获取最大值和最小值
    ...
    //各个小时的会话个数
    val preDayAndHourOfSessionCount: Array[((String, Int), Int)] = dayAndHour2SessionLengthRDD
      .map(tuple => (tuple._1, 1))
      .reduceByKey(_ + _)
      .collect()

    //val dfdf: RDD[((String, Int), Long)] = dayAndHour2SessionLengthRDD.reduceByKey(_ + _)
    //各个小时的总的会话长度
    ...
    val top10CategoryIDAndCountRDD: RDD[(String, (Int, Int, Int))] = sessionID2RecordsRDD
      .flatMap {
        case (sessionID, records) => {
          ...
          // 返回值
          //val dfd: Iterable[((String, Int), Int)] = iter.map(key => (key, 1))
          iter.map(key => (key, 1))
        }
      }
      .reduceByKey(_ + _)//对相同的品类的相同操作进行聚集计数。
      .map(tuple => (tuple._1._2, (tuple._1._1, tuple._2)))//(操作类别，(品类ID,操作次数))
      .groupByKey() // 按照操作类别进行分组，形成(0,1,2)三组
      .flatMap {
      case (flag, iter) => {
        ...
        val top10Category: List[(String, Int)] = iter
          .toList
          .sortBy(_._2)
          .takeRight(10)//根据操作类别，取出前10个操作次数最多的品类ID
...
g
  ...
  /**
    ...
    * 常见的过滤参数:data、sex、age、processional、address、最近搜索的关键词、订单的相关信息(最高订单金额、
    * 最高订单商品单价)......
    * @param sqlContext
    * @param taskParam
    * @return
  ...
  def getActionRDDByFilter(sqlContext: SQLContext, taskParam: JSONObject): RDD[UserVisitSessionRecord] ={
    ...
    //val dfd: Option[String] = startDate.map(v => s" uva.date >= '${v}' ")
    ...
    println(s"==================\n${hql}\n===========================")
    //hql 的执行得到DataFrame
    val df: DataFrame = sqlContext.sql(hql)
    //DataFrame转换为RDD
